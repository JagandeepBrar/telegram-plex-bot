...
def checkAdmin(update):
    if(not select.isUserStatus(update.message.chat_id, constants.ACCOUNT_STATUS_ADMIN)):
        update.message.reply_text(constants.ACCOUNT_UNAUTHORIZED, parse_mode=telegram.ParseMode.MARKDOWN)
        return False
    return True
...
def checkUnverified(update):
    if(select.isUserStatus(update.message.chat_id, constants.ACCOUNT_STATUS_UNVERIFIED)):
        update.message.reply_text(constants.ACCOUNT_UNVERIFIED, parse_mode=telegram.ParseMode.MARKDOWN)
        return True
    return False
...
def checkBanned(update):
    if(select.isUserStatus(update.message.chat_id, constants.ACCOUNT_STATUS_BANNED)):
        update.message.reply_text(constants.ACCOUNT_BANNED, parse_mode=telegram.ParseMode.MARKDOWN)
        return True
    return False
...
def checkRestricted(update, command):
    if(select.isUserStatus(update.message.chat_id, constants.ACCOUNT_STATUS_RESTRICTED)):
            update.message.reply_text(constants.ACCOUNT_RESTRICTED, parse_mode=telegram.ParseMode.MARKDOWN)
            return True
    return False
